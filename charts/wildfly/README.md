...
| `deploy.ingress` | Configuration specific to the creation of a `Ingress` resource to expose the application | - | - |
...
| `deploy.ingress.enabled` | Determines if a `Ingress` configuration should be created. | `false` | Allows clients outside of Kubernetes to access your application |
...
| `deploy.ingress.tls.enabled` | Determines if Ingress  should be TLS-encrypted. If set to `true`, Ingress will use the secure service to acess to the deployment. | `false`| [Kubernetes Documentation](https://kubernetes.io/docs/concepts/services-networking/ingress/#tls) |
